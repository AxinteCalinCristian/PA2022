...
public class PrimAlgorithm {
	...
	/**
	 * @return a list of the streets representing an MST
	 */
	public List<Street> getSolution() {
		...
		return getMSTStreets(parent);
	}
	
	/**
	 * @return a list of the streets representing an MST
	 */
	private List<Street> getMSTStreets(Map<Intersection, Intersection> parent)
	{
		List<Street> mst_streets = new LinkedList<>();
		List<Street> streets = city.getStreets();
	    for(Intersection i : adjacency_list.keySet()) {
	    	if(parent.get(i) != null) {
	    		mst_streets.addAll(streets.stream().filter(s -> (s.getLength() == adjacency_list.get(i).get(parent.get(i)) 
	    				&& (s.getAdjacent_intersections().first().equals(parent.get(i))
	    				&& s.getAdjacent_intersections().second().equals(i)) 
	    				|| (s.getAdjacent_intersections().second().equals(parent.get(i))
	    	    			&& s.getAdjacent_intersections().first().equals(i)))).toList());
	    	}	
	    }
	    return mst_streets;
	}
